...
var GameTranslations = translations{
	"ru": {
		...
		"duel challenge":      {message: SimpleMessage("%challenger вызывает %opponent на дуэль!")},
		"duel accept button":  {message: SimpleMessage("Принять")},
		"duel decline button": {message: SimpleMessage("Отклонить")},
		"duel accepted": {
			oneOf: []oneOf{
				{message: SimpleMessage("%challenger ⚔️ %opponent — вызов принят. Оружие наготове...")},
				{message: SimpleMessage("%challenger ⚔️ %opponent — дуэль начинается. Кто-то сегодня пожалеет.")},
				{message: SimpleMessage("%challenger ⚔️ %opponent — два ствола, одна пуля. Начинаем.")},
				{message: SimpleMessage("%challenger ⚔️ %opponent — разворачивайтесь и стреляйте.")},
				{message: SimpleMessage("%challenger ⚔️ %opponent — честь требует крови. Приступим.")},
			},
		},
		"duel result": {
			oneOf: []oneOf{
				{message: SimpleMessage("Дуэль окончена. %loser повержен.")},
				{message: SimpleMessage("%loser не успел выстрелить первым.")},
				{message: SimpleMessage("Пуля нашла %loser. Дуэль завершена.")},
				{message: SimpleMessage("%loser проиграл дуэль. Честь восстановлена.")},
				{message: SimpleMessage("Выстрел — и %loser падает.")},
			},
		},
		"duel declined": {
			oneOf: []oneOf{
				{message: SimpleMessage("%challenger ⚔️ %opponent — вызов отклонён. Трусость? Мудрость?")},
				{message: SimpleMessage("%challenger ⚔️ %opponent — %opponent решил не рисковать. Разумно.")},
				{message: SimpleMessage("%challenger ⚔️ %opponent — дуэль отменена. Сегодня пас.")},
			},
		},
		"duel is bot":       {message: SimpleMessage("Бот в дуэли не участвует. Найди живого противника.")},
		"duel self":         {message: SimpleMessage("Стреляться с самим собой? Это даже для нас слишком.")},
		"duel no target":    {message: SimpleMessage("Укажи противника: /ngduel @username или реплаем на сообщение")},
		"duel unknown user": {message: SimpleMessage("Не знаю такого. Вызови реплаем на сообщение.")},
		"duel not in chat":  {message: SimpleMessage("Этого игрока нет в чате.")},
		"duel not your":     {message: SimpleMessage("Эта дуэль не для тебя.")},
