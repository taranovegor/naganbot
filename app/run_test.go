...
func TestRunCapsTheNumberOfConcurrentlyExecutingHandlers(t *testing.T) {
	...
	// The pool is now saturated: sending one more update must block until a
	// handler finishes and frees a slot — this is the backpressure guarantee.
...
func TestRunCancellationReturnsPromptlyEvenWhenThePoolIsSaturated(t *testing.T) {
	...
	// The pool is fully saturated and every handler is blocked forever (until
	// release, which this test never sends before the assertion below). If the
	// loop only knew how to wait for a free slot, cancellation would never be
	// noticed and Run would hang well past its own shutdown timeout.
	cancel()
